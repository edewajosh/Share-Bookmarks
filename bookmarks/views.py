...
from django.contrib.auth.decorators import login_required#Restricts only logged in users to submit bookmarks
...
def _bookmark_save(request, form):
	...
	for tag_name in tag_names:
		tag, dummmy = Tag.objects.get_or_create(name=tag_name)
...
def tag_page(request, tag_name):
	...
	bookmarks = tag.bookmark.order_by('-id')
	variables = dict({
		'bookmarks': bookmarks,
		'tag_name': tag_name,
		'show_tags': True,
...
def search_page(request):
	...
	if ('query') in request.GET:
		...
		if query:
			...
			bookmarks = \
				Bookmark.objects.filter (title=query)[:10]
			variables = dict({
				'form':form,
				'bookmarks':bookmarks,
...
def friend_invite(request):  
	if request.method == 'POST':    
		...
		if form.is_valid():      
			...
			try:        
			 	invitation.send()
			 	messages.SUCCESS(request, 'An invitation was sent to %s.' % invitation.email)             
			except:    
			    messages.ERROR(request, 'There was an error while sending the invitation.')   
